      ...
      ScrollView {
        VStack(spacing: 24) {
          ...
          VStack(spacing: 10) {
            ...
            Text(
              viewModel.selectedRating > 0
                ? String(format: "%.0f / 5", viewModel.selectedRating)
                : "Tap to rate"
            )
            .font(.system(size: 15, weight: .medium))
            .foregroundStyle(viewModel.selectedRating > 0 ? .orange : .secondary)
            .animation(.easeInOut(duration: 0.15), value: viewModel.selectedRating)
          }
          ...
          VStack(alignment: .leading, spacing: 10) {
            ...
            HStack {
              Spacer()
              Text("\(viewModel.reviewText.count) / 1000")
                .font(.system(size: 12))
                .foregroundStyle(viewModel.reviewText.count > 1000 ? .red : .secondary)
                .padding(.trailing, 20)
            }
          }
          Button {
            ...
            .background(
              Capsule()
                ...
                .shadow(color: viewModel.buttonColor.opacity(0.4), radius: 10, x: 0, y: 5)
            )
          }
          .padding(.horizontal, 20)
          .disabled(!viewModel.canSubmit)
          .animation(.spring(response: 0.3, dampingFraction: 0.7), value: viewModel.submitted)
